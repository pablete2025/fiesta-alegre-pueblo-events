...
const FestivalHeader = () => {
  ...
  return <header className="bg-gradient-to-r from-festival-yellow via-festival-orange to-festival-red shadow-lg sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="flex items-center space-x-4">
            {!user ? <Link to="/login">
                ...
                {isAdmin && location.pathname !== '/admin' && <Link to="/admin">
                    <Button variant="outline" className="bg-white/20 border-white/30 text-white hover:bg-white/30">
                      <Settings className="w-4 h-4 mr-2" />
                      Panel Admin
                    </Button>
                  </Link>}
                <Button onClick={logout} variant="outline" className="bg-white/20 border-white/30 text-white hover:bg-white/30">
